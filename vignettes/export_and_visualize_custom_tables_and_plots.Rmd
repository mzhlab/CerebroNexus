...
CerebroNexus lets you export and visualize custom data alongside scRNA-seq data. You can attach tables and plots (made with ggplot2) to a data set. This article shows how to do it.

# Tables

CerebroNexus can export and visualize tables of any format as long as they are a `data.frame`. Dynamic column formatting and colour highlighting can further enhance the table content.

## Marker genes and enriched pathways

Assume you have a `Seurat` object but generated tables of differentially expressed genes and enriched pathways using other tools or methods. To export those tables, place them according to a "method" and "name" scheme. In this example, we use a marker-gene table called `custom_table`:
...
class: Cerebro
exporter package version: 5.1.0
