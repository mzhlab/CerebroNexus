# Export a data set in SCE format

## Overview

Here, I explain how one can export a data set stored in the
[`SingleCellExperiment`/`SCE`](https://bioconductor.org/packages/release/bioc/html/SingleCellExperiment.html)
format to visualize it in Cerebro.

The workflow used here is loosely put together from the
[`scran`](https://bioconductor.org/packages/release/bioc/vignettes/scran/inst/doc/scran.html)
and
[`scater`](https://bioconductor.org/packages/3.11/bioc/vignettes/scater/inst/doc/overview.html)
examples. Do not use this workflow as a reference for how to properly
process your data.
...
Now, we create a `SingleCellExperiment` object and filter out cells with
less than `50` transcripts or fewer than `10` expressed genes. Those
cut-offs are only reasonable for this example data set and will likely
need to be adjusted in a real data set. Then, we…
...
our `SCE` object, e.g. an experiment name and the species. The info we
store there will later be collected for visualization in Cerebro.
...
set. Here, marker genes are identified for clusters and cell types using
the `findMarkers()` function from the `scran` package. The results are
separate tables for the respective subgroups, which we merge together in
order to make them compatible with Cerebro.
